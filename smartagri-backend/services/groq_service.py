...
SYSTEM_PROMPT_QUICK = """You are an expert agricultural advisor specializing in Indian organic and natural farming.
Provide accurate, practical advice for crop diseases, pests, soil health, irrigation, fertilizers, and farming.
...
async def ask_groq(question: str, is_comprehensive: bool = False, profile: dict = None) -> str:
    """
    Asynchronously query Groq LLaMA for agricultural advice.
    Injects dynamic market, weather, and subsidy context based on the query.
    """
    ...
    try:
        ...
        print(f"[WARNING][Groq Service] Error: {e}. Falling back to Demo Simulation...")
        q_lower = question.lower()
        if "blight" in q_lower or "tomato" in q_lower or "disease" in q_lower or "disease" in q_lower:
            return (
                "The issue is identified as Early Blight with moderate severity. "
                "Organic remedies: Spray 5% Neem Seed Kernel Extract (NSKE) at weekly intervals or apply Sour Buttermilk spray. "
                "Tomato mandi price is ₹3,200 per quintal and trending upwards; hold harvest for 3 days. "
                "Recommending PM-KISAN subsidy for organic inputs."
            )
        elif "market" in q_lower or "price" in q_lower or "mandi" in q_lower:
            return "Mandi price for tomato is ₹3,200 per quintal. The trend is rising. Recommendation: Hold harvest for 3 days."
        elif "subsidy" in q_lower or "scheme" in q_lower or "government" in q_lower:
            return "The PM-KISAN scheme provides Rs. 6,000 direct income support. PKVY provides Rs. 50,000 per hectare for organic farming transition."
        else:
            return "The issue is identified as Early Blight with moderate severity. Organic remedies: Spray 5% Neem Seed Kernel Extract (NSKE)."
...
async def parse_intent_and_advisory(question: str, profile: dict = None, tool_context: dict = None) -> dict:
    """
    Use Groq LLaMA to semantically parse intent and formulate a response using pre-fetched context.
    """
    model_name = "llama-3.3-70b-versatile"
    context = ""
    if profile:
    ...
    try:
        ...
        print(f"[WARNING][Groq Service] Intent parse failed: {e}. Falling back to keyword logic.")
        q_lower = question.lower()
        if any(w in q_lower for w in ["market", "price", "mandi", "cost", "sell"]):
            return {
                "intent": "AskPrice",
                "action": "AnswerAndSuggestNavigation",
                "target": "/market",
                "tool_used": "get_market_price",
                "answer": "Tomato price is ₹26 per kilogram and rising. Coimbatore Vegetable Mandi is the best buyer.",
                "suggest_label": "View Detailed Market Trends"
            }
        elif any(w in q_lower for w in ["pest", "disease", "bug", "alert", "insect"]):
            return {
            ...
            return {
                ...
                "tool_used": "get_crop_recommendation",
                "answer": "We recommend planting Groundnut this season for highest profit margins.",
                "suggest_label": "Explore Crop Recommendations"
            }
        elif any(w in q_lower for w in ["home", "dashboard", "back"]):
            return {
                "intent": "OpenPage",
                "action": "Navigate",
                "target": "/home",
                "tool_used": "none",
                "answer": "Navigating back to your home dashboard.",
                "suggest_label": None
            }
        else:
            return {
                ...
                "target": None,
                "tool_used": "none",
                "answer": "To boost soil microbiology, apply fermented Jivamrita once every two weeks.",
                "suggest_label": None
            }